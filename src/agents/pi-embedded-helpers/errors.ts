...
const ERROR_PATTERNS = {
  ...
  auth: [
    ...
    "expired",
    "token has expired",
    /["']?(?:status|code)["']?\s*[:=]\s*401\b|\bhttp\s*401\b|\berror(?:\s+code)?\s*[:=]?\s*401\b|\b(?:got|returned|received)\s+(?:a\s+)?401\b|^\s*401\s+unauthorized/i,
    /["']?(?:status|code)["']?\s*[:=]\s*403\b|\bhttp\s*403\b|\berror(?:\s+code)?\s*[:=]?\s*403\b|\b(?:got|returned|received)\s+(?:a\s+)?403\b|^\s*403\s+forbidden/i,
