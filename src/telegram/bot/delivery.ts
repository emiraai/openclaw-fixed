...
export async function resolveMedia(
  ...
  // Only relax SSRF / enable direct disk reads when the *account* explicitly
  // sets apiRoot.  The TELEGRAM_API_ROOT env-var fallback routes API calls but
  // must NOT silently weaken SSRF for every account.
  const isLocal = Boolean(apiRoot?.trim()) && isCustomTelegramApi(apiBase);
